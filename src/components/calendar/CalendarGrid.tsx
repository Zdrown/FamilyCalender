        ...
        {days.map((day) => {
          ...
          return (
            <div
              key={dayStr}
              className={`
                bg-bg-card p-0.5 md:p-2 wall:p-3 min-h-[44px] md:min-h-[80px] wall:min-h-[120px] overflow-hidden
                ${today ? 'ring-2 ring-inset ring-accent-primary/30' : ''}
              `}
            >
              <div className={`
                inline-flex items-center justify-center w-5 h-5 md:w-7 md:h-7 wall:w-9 wall:h-9 rounded-full mb-0 md:mb-1
                font-body text-[10px] md:text-sm wall:text-base font-medium
              ...
              <div className="space-y-0.5 md:space-y-1">
                ...
                {dayEvents.length > (viewMode === 'month' ? 3 : 8) && (
                  <p className="text-[8px] md:text-xs text-text-muted font-body pl-0.5 md:pl-2">
                    +{dayEvents.length - (viewMode === 'month' ? 3 : 8)}
                  </p>
                )}
              </div>
